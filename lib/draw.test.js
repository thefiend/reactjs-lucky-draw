...
describe("verifyDraw", () => {
  ...
  // A fixed seed, because "the winners came out different" is not a guarantee:
  // a shorter and a longer list drawing from the same keystream can land on the
  // same names by chance, roughly once in thirty draws for a list this size. What
  // is guaranteed is the fingerprint, which the next test covers.
  it("rejects a draw replayed against a different list", async () => {
    const seed = "00000000000000000000000000000000";
    const result = await runDraw({ entries: NAMES, seed, count: 2 });
    await expect(
      verifyDraw({ entries: [...NAMES, "Mallory"], seed, winners: result.winners })
    ).resolves.toBe(false);
  });

  it("always publishes a different fingerprint once the list is edited", async () => {
    const result = await runDraw({ entries: NAMES, count: 2 });
    const padded = await runDraw({
      entries: [...NAMES, "Mallory"],
      seed: result.seed,
      count: 2,
    });

    expect(padded.listHash).not.toBe(result.listHash);
    expect(padded.serial).not.toBe(result.serial);
  });
});
